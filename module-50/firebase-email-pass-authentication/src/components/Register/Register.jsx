  ...
  return (
    <div className=" mx-auto">
      ...
      <form
        ...
        className="mx-auto flex flex-col gap-4 p-5 bg-gray-200 rounded-xl w-[350px]"
      >
        <input
        ...
        <div className="flex relative">
          <input
            ...
            className="h-12 pl-4 w-full rounded-lg"
            name="password"
          ...
          >
            {showPass ? <FaRegEye></FaRegEye> : <RxEyeClosed></RxEyeClosed>}
          </span>
        </div>
        <div>
      ...
      <div className="w-[330px]  mx-auto ">
        <p className="flex">
          ...
          <NavLink to="/logIn" className="text-blue-500 underline ml-1">
            LogIn
          </NavLink>
        </p>
      </div>
      {submitError && (
        <p className=" text-red-600 text-center my-5">{submitError}</p>
      )}
      {submitSuccess && (
        <p className="text-center text-green-700 my-5">{submitSuccess} 😀 </p>
      )}
      {verifyEmail && (
        <p className="text-center text-green-700 my-5">{verifyEmail} </p>
      )}
      {verifyError && (
        <p className=" text-red-600 text-center my-5">{verifyError}</p>
      )}
    </div>
  );
};
